...
#anchors = [47, 20,  58, 26,  73, 26,  76, 33,  86, 41, 114, 53] #v3-tiny
anchors = [42, 20,  51, 18,  54, 23,  60, 28,  72, 25,  73, 35,  84, 31,  90, 42, 121, 59] #v3
...
def create_model(input_shape, anchors, num_classes):
    ...
    model_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
        arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.})(
...
def create_test_model(model_path):
    ...
    if len(anchors) == 6:
        ...
        model = yolo_body(image_input, len(anchors)//3, num_classes)
...
model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
    ...
    epochs=epochs,
    initial_epoch=0,
    callbacks=[reduce_lr, early_stopping, restore_best_weights],
    verbose=verbose,
    workers=workers)
...
#%% predict
yolo_model = create_test_model(output_model_path)
